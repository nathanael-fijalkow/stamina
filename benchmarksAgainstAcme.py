...
parser.add_argument('acme', help='path to the AcmeML executable')
...
for i in range(fr,args.n+1):
    for x in range(0,args.repeat):
        ...
        if maybeTimeout(s):
            ...
            monsize = re.search(r'has (\d+)',stamina_out).group(1)
        
            print 'We picked a monoid of size ' + monsize
            x1=time_out.replace('"','').strip()
            print 'Stamina took ' + x1
    
            s = subprocess.Popen(['time','-f "%U"','./'+args.acme,'-mma',tempFile,'-silent'],
                             stdout=subprocess.PIPE,stderr=subprocess.PIPE)
            if maybeTimeout(s):
